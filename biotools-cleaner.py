...
if not flagged.empty:
    ...
    figure, (ax1) = plt.subplots(1, 1, figsize=(5,5))
    plt.plot(np.sort(data['Meas. M/z'].apply(float)), invfn(np.sort(data['Meas. M/z'].apply(float)), *popt))
    plt.scatter(flagged['Meas. M/z'].apply(float), flagged['Dev.(ppm)'].apply(float), c=flagged['Score'], norm=colors.LogNorm(vmin=1, vmax=max(10,max(flagged['Score']))))
    plt.colorbar(label='Score')
    plt.xlabel('M/z')
    plt.ylabel('deviation (ppm)')
    plt.ylim(-30,30)
...
if not yellow.empty:
    ...
    figure, (ax1) = plt.subplots(1, 1, figsize=(5,5))
    plt.plot(np.sort(data['Meas. M/z'].apply(float)), invfn(np.sort(data['Meas. M/z'].apply(float)), *popt))
    plt.scatter(yellow['Meas. M/z'].apply(float), yellow['Dev.(ppm)'].apply(float), c=yellow['Score'], norm=colors.LogNorm(vmin=1, vmax=max(10,max(flagged['Score']))))
    plt.colorbar(label='Score')
    plt.xlabel('M/z')
    plt.ylabel('deviation (ppm)')
    plt.ylim(-30,30)
